...
class Sensor {
  public:
    Sensor(uint16_t update_frequency, uint16_t accuracy, uint16_t precision, boolean calibrated)
        : _update_frequency(update_frequency), _accuracy(accuracy), _precision(precision), _calibrated(calibrated){};
    ...
    virtual enum sensor_status status() {
        return _status;
    };
    ...
    virtual boolean ready_for_update() {
        if(_status == SENSOR_NOT_FOUND)
            return false;
        if((millis() - _last_checked) >= _update_frequency)
            return true;
        else
            return false;
    };
